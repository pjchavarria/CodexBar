...
- Show Codex Weekly only. Give each Claude account one Weekly group with the normal Weekly bar followed by yellow
  Session and Fable bars.
- Do not render usage dashboards inside account or provider sections. Render one combined cost/token KPI and chart
  model at the bottom of the entire overview, without dashboard detail lines. Aggregate retained typed usage values
...
are clean; confirm that every account identity appears and that the one bottom dashboard covers every available typed
cost source without duplicating provider sections.
